    ...
    if total_packets > packet_count * threshold:
        ...
        for packet in packets:
            if scapy.IP in packet:
                source_ips.append((packet[scapy.IP].src, time.strftime("%Y-%m-%d %H:%M:%S", time.gmtime())))

        if source_ips:
            print("[+] Source IPs maybe triggering the DoS attack:")
            for src_ip in set([ip[0] for ip in source_ips]):
                print(src_ip)

            # Save data to CSV
            save_to_csv(source_ips, "wifi_dos_data.csv")
        else:
